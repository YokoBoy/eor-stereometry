...
async function init() {
  ...
  const videosTable = isPostgres
    ? 'CREATE TABLE IF NOT EXISTS videos (id SERIAL PRIMARY KEY, title TEXT NOT NULL, section_id INTEGER, topic TEXT NOT NULL, description TEXT, content TEXT, url TEXT NOT NULL, duration INTEGER, is_free INTEGER DEFAULT 1, published_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(section_id) REFERENCES sections(id))'
  ...
  const assignmentsTable = isPostgres
    ? 'CREATE TABLE IF NOT EXISTS assignments (id SERIAL PRIMARY KEY, video_id INTEGER, type TEXT NOT NULL, title TEXT NOT NULL, content TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(video_id) REFERENCES videos(id))'
    : 'CREATE TABLE IF NOT EXISTS assignments (id INTEGER PRIMARY KEY AUTOINCREMENT, video_id INTEGER, type TEXT NOT NULL, title TEXT NOT NULL, content TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(video_id) REFERENCES videos(id))';

  const submissionsTable = isPostgres
    ? 'CREATE TABLE IF NOT EXISTS submissions (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL, assignment_id INTEGER NOT NULL, content TEXT, grade INTEGER, feedback TEXT, status TEXT DEFAULT \'pending\', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(assignment_id) REFERENCES assignments(id))'
    : 'CREATE TABLE IF NOT EXISTS submissions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, assignment_id INTEGER NOT NULL, content TEXT, grade INTEGER, feedback TEXT, status TEXT DEFAULT "pending", created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(assignment_id) REFERENCES assignments(id))';

  const progressTable = isPostgres
    ? 'CREATE TABLE IF NOT EXISTS progress (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL, video_id INTEGER NOT NULL, status TEXT NOT NULL, watched_seconds INTEGER DEFAULT 0, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, video_id), FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(video_id) REFERENCES videos(id))'
